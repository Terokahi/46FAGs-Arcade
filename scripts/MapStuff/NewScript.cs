		...
		private void UpdateTileMapLayers(Rect2I pos, int layerID)
		{
			...
			for (int x = pos.Position.X; x < pos.Size.X + 1; x++)
			{
				for (int y = pos.Position.Y; y < pos.Size.Y + 1; y++)
				{
					...
					int bl,tr,tl;
					try { bl = map[x - 1, y, layerID]; } catch (IndexOutOfRangeException) { bl = getTS_ID["void"]; }
					try { tr = map[x, y - 1, layerID]; } catch (IndexOutOfRangeException) { tr = getTS_ID["void"]; }
					try { tl = map[x - 1, y - 1, layerID]; } catch (IndexOutOfRangeException) { tl = getTS_ID["void"]; }

					//Determine the atlas vector based on the neighboring cells
					int atlasVector = 0;
					if (br != getTS_ID["void"]) atlasVector |= (int)Location.LOW_RIGHT;
					if (tr != getTS_ID["void"]) atlasVector |= (int)Location.TOP_RIGHT;
					if (tl != getTS_ID["void"]) atlasVector |= (int)Location.TOP_LEFT;
					if (bl != getTS_ID["void"]) atlasVector |= (int)Location.LOW_LEFT;
